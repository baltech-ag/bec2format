...
from bec2format.crypto import AES128, create_AES128, \
    create_public_ecc_key_from_der_fmt, PublicEccKey, \
    generate_private_ecc_key, PrivateEccKey
from bec2format.configid import ConfigId, MissingDeviceSettingsNameError, \
...
def crc8404B(data, startValue: int = 0xFFFF) -> int:
    """
    used for MIC Transponder, Mifare ...
    generator Polynom: x^16 + x^12 + x^5 + 1
    """
    curCrc = int(startValue)
    for c in data:
        byte = c ^ (curCrc & 0xFF)
        byte ^= (byte << 4) & 0xFF
        curCrc = (curCrc >> 8) ^ (byte << 8) ^ (byte << 3) ^ (byte >> 4)
    return curCrc
...
class SoftwareCustKeyEncryptor(AesEncryptorMixin, CustKeyEncryptor):
    ...
    def __init__(self, crypto_key: bytes, customer_key: bytes | None = None, customer_key_pos: int | None = None) -> None:
    ...
    def decrypt(self, ciphertext):
        ...
        if self.customer_key:
            ...
            plaintext = bytearray(plaintext)
            plaintext[ck_pos:ck_pos+CUSTOMER_KEY_SIZE] = bytes([0x00] * CUSTOMER_KEY_SIZE)
...
class EccEncryptor(Encryptor):
    ...
    def __init__(self, key_selector: int = KEYSEL_FW_STD, public_key: PublicEccKey | None = None) -> None:
        super().__init__()
        self.key_selector = key_selector
        self.public_key = public_key or create_public_ecc_key_from_der_fmt(self.DEFAULT_PUBLIC_KEYS[key_selector])
...
class EccDecryptor(EccEncryptor):
    ...
    def decrypt(self, ciphertext: bytes) -> bytes:
        ...
        der_header = bytes.fromhex("30 59 30 13 06 07 2A 86 48 CE 3D 02 01 06 08 2A 86 48 CE 3D 03 01 07 03 42 00 04")
...
class AuthBlock:
    ...
    @classmethod
    def select_encryptor(cls, ext_encryptors: list[Encryptor] | None = None,
                         fallback_encryptor: Encryptor | None = None,
                         encryptor_filter: Callable[[Encryptor], bool] | None = None) -> Encryptor:
    ...
    def pack(self, session_key: bytes, ext_encryptors: list[Encryptor] | None = None) -> bytes:
        raise NotImplementedError()

    @classmethod
    def unpack(cls, raw: bytes, ext_encryptors: list[Encryptor] | None = None) -> tuple["AuthBlock", bytes]:
        raise NotImplementedError()
...
class InitEccAuthBlock(AuthBlock):
    ...
    def pack(self, session_key: bytes, ext_encryptors: list[Encryptor] | None = None) -> bytes:
        ...
        return self.key_selector.to_bytes(1, byteorder="big") + encryptor.encrypt(session_key)
    ...
    def unpack(cls, raw: bytes, ext_encryptors: list[Encryptor] | None = None) -> tuple[AuthBlock, bytes]:
        ...
        encryptor = cls.select_encryptor(
            ext_encryptors,
            encryptor_filter=lambda e: e.key_selector == key_selector)
...
class Bec2File:
    ...
    def __init__(self, bf3file: Bf3File, auth_blocks: Iterable[AuthBlock] = (), session_key: bytes | None = None):
    ...
    def write_file(self, bf3file: str | TextIO, ext_encryptors: list[Encryptor] | None = None) -> None:
    ...
    @classmethod
    def read_file(cls, bf3file: str | TextIO, ext_encryptors: list[Encryptor] | None = None, check_cmac: bool = True) -> "Bec2File":
    ...
    @classmethod
    def unpack_auth_blocks(cls, raw_rdr: BytesIO, ext_encryptors: list[Encryptor]) -> tuple[Iterable[AuthBlock], bytes]:
    ...
    def derive_auth_blocks_from_config(self, config: dict, cust_key_support: bool = False) -> None:
        if cust_key_support:
