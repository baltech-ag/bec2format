...
if __name__ == '__main__':
    ...
    USE_HW_ENCRYPTOR = False

    # set environment vars (the content in these vars is highly sensible and
    # thus MUST NEVER BE HARDCODED INTO PYTHON SCRIPTS TO AVOID ACCIDENTIAL
    # LEAKING INTO DELIVERED CODE!!!):
    # - BAL_CONFIG_SUPER_KEY to global variable tlv_aes_crypto_key
    #   (see Bros2FW/config/tlvparser.c)
    # - BAL_CUSTOMER_KEY to customer key. If unknown see
    #   "M:\20_Produktion\_PROJEKTDATEN\customer_keys.xml"
    if USE_HW_ENCRYPTOR:
        from baltech.brp import BrpDevice, HidPort
        encryptors = [HardwareCustKeyEncryptor(BrpDevice(HidPort()))]
    else:
        custkey = os.environ.get('BAL_CUSTOMER_KEY')
        soft_encryptor = SoftwareCustKeyEncryptor(
            buf(os.environ.get('BAL_CONFIG_SUPER_KEY', "1234" * 8)),
            buf(custkey) if custkey else None)

        testdata = buf.raw("Das ist ein Testtext")
        encrypted_data = soft_encryptor.encrypt(testdata)
        assert soft_encryptor.decrypt(encrypted_data) == testdata

        encryptors = [soft_encryptor]

    priv_key = PrivateEccKey.createFromRawBinFmt(
        's\xee\xf1w\xa2o\xe9\xbd\xbb\xe96ylh\xc9imq' \
        '\x07e\xb6/\x87\xda\x1f\xf1\xba\x83E\xd9\xc9o',
        '\xae\x06!\xa0;\xa2\xd5\xae\x94\xd7\xa2\xf1s\x8b\x8b\xef>\xf2\xb6\xfe' \
        '\x1e\x14\xb3"\xb9LJFy\x0f\xcdo3\x15\x1c\x1d\x16\x1a\xaa\x98 ' \
        '"+JWy\x90\x16Q\xcck^^"\xf0\xe82\xba\xb1\xa8u.J\xb7')
    encryptors += [EccDecryptor(EccDecryptor.KEYSEL_FW_STD, priv_key)]

    config_security_code = buf("45" * CONFIG_SECURITY_CODE_SIZE)
    config = {
        (0x1111, 0x22): buf("33 33 33"),
        (0x1111, 0x77): buf("55 66 77 88"),
        (0x0202, 0x82): config_security_code,
        (0x0620, 0x01): buf.int32(10234),
        (0x0620, 0x05): buf.int16(5678),
        (0x0620, 0x02): buf.int16(6789),
        (0x0620, 0x07): buf.int8(9),
        (0x0620, 0x06): buf(raw="Testname"),
    }
    ...
    bec2 = Bec2File(
        Bf3File(
            ...
            components=[
                Bf3Component({0xC1: buf("11 22 33"), 0xC3: buf("12 33")}, buf(list(range(0x100)))),
            ],
        ),
        session_key=buf("44 55 16 FF 12 6F 57 02 D7 03 44 A0 94 33 44 55")
    )
